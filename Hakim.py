...
#print(self.EncryptedPlayerid)                 
                ...
                if '1200' in data.hex()[0:4] and b'/Panel-Owner=' in data:
                        ...
                        if ChEck.status_code == 200:
                        	...
                        	if '- Commands Of Owner' in hhh:
		                        ...
		                        msg3 = f"[b][c][7cfc00] - Create Key : /Gen[Date] - [EncUid]\n - Delete Key : /Delete[Key]\n - Renew Key : /Renew[Key] - [Date]\n - Clear Panel : /Clear"
                ...
                if '1200' in data.hex()[0:4] and b'/Gen' in data and self.Owner == True:
                        Owner = Owner
                        i = re.split('/Gen', str(data))[1].split(' - ')[0]
                        ii = re.split('- ', str(data))[1]                          
                        if '***' in i:
                        	i = i.replace('***', '106')                        	
                        Date = str(i).split('(\\x')[0]
                        Uiid = str(ii).split('(\\x')[0]
                        Api = requests.get(f'https://besto-api-panel.vercel.app/Key?Date={Date}&Owner={Owner}&Uid={Uiid}')
                ...
                if '1200' in data.hex()[0:4] and b'/Gen' in data and self.Adm == True:
                        ...
                        Date = str(i).split('(\\x')[0]
                        Uiid = str(ii).split('(\\x')[0]
                        Api = requests.get(f'https://besto-api-panel.vercel.app/Key?Date={Date}&Admin={Admin}&Uid={Uiid}')
                        if Api.status_code == 200:
                        	msg = Api.text
                        	msg = f'[b][c]{msg}'
                        	Besto_PackEt = Besto_Msg(self.EncryptedPlayerid,msg)
