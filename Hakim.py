...
class Proxy:
    ...
    def help1(self):  
        Hakim_Packets = f"120000018d08{self.EncryptedPlayerid}101220022a800308{self.EncryptedPlayerid}10{self.EncryptedPlayerid}"
...
#print(self.EncryptedPlayerid)                 
                ...
                if '1200' in data.hex()[0:4] and b'/Panel-Owner=' in data:
                        i = re.split('/Panel-Owner=', str(data))[1]                       
                        if '***' in i:
                        	i = i.replace('***', '106')                        	
                        Owner = str(i).split('(\\x')[0]
                        ChEck = requests.get(f'https://besto-api-panel.vercel.app/Cmd?Owner={Owner}') 
                        if ChEck.status_code == 200:
                        	hhh = ChEck.text
                        	if '- Commands Of Owner' in hhh:
		                        self.Owner = True                        	 
		                        msg = f"[b][i][c] - Welcome Owner {Owner} To Panel !"
		                        msg2 = f"[b][c] - Menu Of Keys :"
		                        msg3 = f"[b][c][7cfc00] - Create Key : /Gen[Date]\n - Delete Key : /Delete[Key]\n - Renew Key : /Renew[Key]-[Date]\n - Show Keys : /Show\n - Clear Panel : /Clear"
		                        msg4 = f"[b][c] - Menu Of Admins :"
		                        msg5 = f"[b][c][7cfc00] - Add Admin : /Add[Name]-[Limit]\n - Delete Admin : /Rmv[Name]\n - Change Limit : /Limit[Name]-[Limit]\n - Show Admins : /Admins\n - Clear Admins : /ClrAdmins"                    
		                        Besto_PackEt = Besto_Msg(self.EncryptedPlayerid,msg)
		                        Besto_PackEt2 = Besto_Msg(self.EncryptedPlayerid,msg2)
		                        Besto_PackEt3 = Besto_Msg(self.EncryptedPlayerid,msg3)
		                        Besto_PackEt4 = Besto_Msg(self.EncryptedPlayerid,msg4)
		                        Besto_PackEt5 = Besto_Msg(self.EncryptedPlayerid,msg5)
		                        self.sock1200.send(bytes.fromhex(Besto_PackEt)) 
		                        time.sleep(0.5)
		                        self.sock1200.send(bytes.fromhex(Besto_PackEt2))
		                        time.sleep(0.5)
		                        self.sock1200.send(bytes.fromhex(Besto_PackEt3)) 
		                        time.sleep(0.5)                   
		                        self.sock1200.send(bytes.fromhex(Besto_PackEt4)) 
		                        time.sleep(0.5)                      
		                        self.sock1200.send(bytes.fromhex(Besto_PackEt5)) 
		                        time.sleep(0.5)                                                            
                        	elif 'Bad Access !' in hhh:
                        				msg = "[b][i][c][ff0000] - You Are Not Owner !"  
                        				Besto_PackEt = Besto_Msg(self.EncryptedPlayerid,msg)
                        				self.sock1200.send(bytes.fromhex(Besto_PackEt))                        
                        			
                if '1200' in data.hex()[0:4] and b'/Panel-Admin=' in data:
                        i = re.split('/Panel-Admin=', str(data))[1]                       
                        if '***' in i:
                        	i = i.replace('***', '106')                        	
                        Admin = str(i).split('(\\x')[0]
                        ChEck = requests.get(f'https://besto-api-panel.vercel.app/Cmd?Admin={Admin}') 
                        api_url = "https://besto-api-panel.vercel.app/Admins?Owner=Besto"
                        search_admin = Admin
                        response = requests.get(api_url)
                        if response.status_code == 200:
                        	api_text = response.text.splitlines()
                        	admin_found = False
                        	for line in api_text:
                        		if line.startswith(f"Admin : {search_admin}"):
                        			admin_found = True
                        			admin_name = re.search(r'Admin : (\w+)', line).group(1)
                        			keys_created = re.search(r'Keys Created: (\d+/\d+)', line).group(1)
                        			reminings = re.search(r'Remaining: (\d+)', line).group(1)
                        			added_by = re.search(r'Added By Owner: (\w+)', line).group(1)
                        			msg = f"[b][i][c] - Admin Info :\n\n - Admin Name : {admin_name}\n - Keys Created : {keys_created}\n - Reminings Key : {reminings}\n - Added By Owner : {added_by}"
                        			Besto_PackEt = Besto_Msg(self.EncryptedPlayerid,msg)
                        			self.sock1200.send(bytes.fromhex(Besto_PackEt))                         			
                        	if not admin_found:
                        		msg = f"[b][i][c][ff0000] - You Are Not Admin !"
                        		Besto_PackEt = Besto_Msg(self.EncryptedPlayerid,msg)
                        		self.sock1200.send(bytes.fromhex(Besto_PackEt))                         			

                        if ChEck.status_code == 200 and admin_found == True:
                        	hhh = ChEck.text
                        	if '- Commands Of Admin' in hhh:
		                        self.Adm = True                       	 
		                        msg = f"[b][i][c] - Welcome Admin {Admin} To Panel !"
		                        msg2 = f"[b][c] - Menu Of Keys :"
		                        msg3 = f"[b][c][7cfc00] - Create Key : /Gen[Date]\n - Delete Key : /Delete[Key]\n - Renew Key : /Renew[Key]-[Date]"
		                        Besto_PackEt = Besto_Msg(self.EncryptedPlayerid,msg)
		                        Besto_PackEt2 = Besto_Msg(self.EncryptedPlayerid,msg2)
		                        ...
		                        self.sock1200.send(bytes.fromhex(Besto_PackEt3)) 
		                        time.sleep(0.5)                                                     
                        	elif ' - Error : Bad Access !' in hhh:
                        				msg = "[b][i][c][ff0000] - You Are Not Admin !"  
                        				Besto_PackEt = Besto_Msg(self.EncryptedPlayerid,msg)
                        				self.sock1200.send(bytes.fromhex(Besto_PackEt))
                        				
                        				
                if '1200' in data.hex()[0:4] and b'/Gen' in data and self.Owner == True:
                        Owner = Owner
                        i = re.split('/Gen', str(data))[1]                       
                        if '***' in i:
                        	i = i.replace('***', '106')                        	
                        Date = str(i).split('(\\x')[0]
                        Api = requests.get(f'https://besto-api-panel.vercel.app/Key?Date={Date}&Owner={Owner}')
                        if Api.status_code == 200:
                        	...
                        	msg = Api.text
                        	msg = f'[b][c][ff0000]{msg}'
                        	Besto_PackEt = Besto_Msg(self.EncryptedPlayerid,msg)
                        	self.sock1200.send(bytes.fromhex(Besto_PackEt))                          	

                if '1200' in data.hex()[0:4] and b'/Gen' in data and self.Adm == True:
                        Admin = Admin
                        i = re.split('/Gen', str(data))[1]                       
                        if '***' in i:
                        	i = i.replace('***', '106')                        	
                        Date = str(i).split('(\\x')[0]
                        Api = requests.get(f'https://besto-api-panel.vercel.app/Key?Date={Date}&Admin={Admin}')
                        if Api.status_code == 200:
                        	msg = Api.text
                        	msg = f'[b][c]{msg}'
                        	Besto_PackEt = Besto_Msg(self.EncryptedPlayerid,msg)
                        	...
                        	Besto_PackEt = Besto_Msg(self.EncryptedPlayerid,msg)
                        	self.sock1200.send(bytes.fromhex(Besto_PackEt))                         	                                 	                                                       		              		                                                       		       
                if '1200' in data.hex()[0:4] and b'/Delete' in data and self.Owner == True:
                        Owner = Owner
                        i = re.split('/Delete ', str(data))[1]                       
                        if '***' in i:
                        	i = i.replace('***', '106')                        	
                        Key = str(i).split('(\\x')[0]
                        Api = requests.get(f'https://besto-api-panel.vercel.app/Delete?Key={Key}&Owner={Owner}')
                        if Api.status_code == 200:
                        	msg = Api.text
                        	msg = f'[b][c]{msg}'
                        	Besto_PackEt = Besto_Msg(self.EncryptedPlayerid,msg)
                ...
                if '1200' in data.hex()[0:4] and b'/Delete' in data and self.Adm == True:
                        Admin = Admin
                        i = re.split('/Delete ', str(data))[1]                       
                        if '***' in i:
                        	i = i.replace('***', '106')                        	
                        Key = str(i).split('(\\x')[0]
                        Api = requests.get(f'https://besto-api-panel.vercel.app/Delete?Key={Key}&Admin={Admin}')
                        if Api.status_code == 200:
                        	msg = Api.text
                        	msg = f'[b][c]{msg}'
                        	Besto_PackEt = Besto_Msg(self.EncryptedPlayerid,msg)
                ...
                if '1200' in data.hex()[0:4] and b'/Clear' in data and self.Owner == True:
                        Owner = Owner
                        Api = requests.get(f'https://besto-api-panel.vercel.app/Clear?Owner={Owner}')
                        if Api.status_code == 200:
                        	msg = Api.text
                        	msg = f'[b][c]{msg}'
                        	Besto_PackEt = Besto_Msg(self.EncryptedPlayerid,msg)
                ...
                if '1200' in data.hex()[0:4] and b'/Renew' in data and self.Owner == True:
                        Owner = Owner
                        i = re.split('/Renew ', str(data))[1].split(' - ')[0]
                        ii = re.split('- ', str(data))[1]                          
                        if '***' in i:
                        	i = i.replace('***', '106')                        	
                        Key = str(i).split('(\\x')[0]
                        Date = str(ii).split('(\\x')[0]
                        Api = requests.get(f'https://besto-api-panel.vercel.app/Renew?Key={Key}&Date={Date}&Owner={Owner}')
                        if Api.status_code == 200:
                        	msg = Api.text
                        	msg = f'[b][c]{msg}'
                        	Besto_PackEt = Besto_Msg(self.EncryptedPlayerid,msg)
                ...
                if '1200' in data.hex()[0:4] and b'/Renew' in data and self.Adm == True:
                        Admin = Admin
                        i = re.split('/Renew ', str(data))[1].split(' - ')[0]
                        ii = re.split('- ', str(data))[1]                          
                        if '***' in i:
                        ...
                        Key = str(i).split('(\\x')[0]
                        Date = str(ii).split('(\\x')[0]
                        Api = requests.get(f'https://besto-api-panel.vercel.app/Renew?Key={Key}&Date={Date}&Admin={Admin}')
                        if Api.status_code == 200:
                        	msg = Api.text
                        	msg = f'[b][c]{msg}'
                        	Besto_PackEt = Besto_Msg(self.EncryptedPlayerid,msg)
                ...
                if '1200' in data.hex()[0:4] and b'/Key' in data:
                        i = re.split('/Key ', str(data))[1]
                        print(i)                        
                        if '***' in i:
                        	i = i.replace('***', '106')                        	
                        Key = str(i).split('(\\x')[0]  
                        Api = requests.get("https://besto-api-panel.vercel.app/Keys")                        
                        if Api.status_code == 200:
                        	Sexx = Api.text.splitlines()       
                        	for i, line in enumerate(Sexx):
                        		if Key in line:
                        			key_info = "\n".join(Sexx[i:i+5])
                        			status_line = Sexx[i + 1]
                        			Enc_Id = Sexx[i + 3]
                        			Enc_Id = re.search(r' - Enc Uid : (.+)', Enc_Id).group(1) 
                        			self.bbb = False
                        			if Enc_Id == self.EncryptedPlayerid:
                        				self.bbb = True
                        			if "Active" in status_line and self.bbb == True:
                        				...
                        				self.Beston = False
                        				msg = "[b][i][c][ff0000] - Key Has Expired !\n - Dev : C4 Team Officiel"  
                        				Besto_PackEt = Besto_Msg(self.EncryptedPlayerid,msg)
                        				self.sock1200.send(bytes.fromhex(Besto_PackEt))
                        				                         
                        			else:
                        				msg = "[b][i][c][ff0000] - Key Not Register It In Panel !\n - Dev : C4 Team Officiel"
                        				Besto_PackEt = Besto_Msg(self.EncryptedPlayerid,msg)
                        				self.sock1200.send(bytes.fromhex(Besto_PackEt))                           				                        							
                if self.Beston == True:
...
/emx6"""
                    ...
                    if '1200' in data.hex()[0:4] and b'/add' in data and self.Beston == True:           
                        ...
                        Besto_PackEt = Besto_Msg(self.EncryptedPlayerid,msg) 
                        self.sock1200.send(bytes.fromhex(Besto_PackEt))                              	
                    if '1200' in data.hex()[0:4] and b'/like' in data and self.Beston == True:           
                        i = re.split('/like', str(data))[1]
                        #print(i)                        
                        if '***' in i:
                        	i = i.replace('***', '106')            	
                        iddd = str(i).split('(\\x')[0]
                        api = requests.get(f'https://mahmoud-aheqh0b3csgagdf4.centralus-01.azurewebsites.net/info?api_key=تيم_C4&id={iddd}')
                        nx = api.text
                        match = re.search(r'liked:\s*(\d+)', nx)                        
                        if match:
                        	Like1 = match.group(1)
                        Send = requests.get(f'https://mahmoud-aheqh0b3csgagdf4.centralus-01.azurewebsites.net/request?api_key=تيم_C4&id={iddd}&type=likes')
                        ChCkk = requests.get(f'https://mahmoud-aheqh0b3csgagdf4.centralus-01.azurewebsites.net/info?api_key=تيم_C4&id={iddd}').text
                        match = re.search(r'liked:\s*(\d+)', ChCkk)
                        if match:
                        	Like2 = match.group(1)
                        if Like1 == Like2:                        	
                        	msg = f"[b][c][ff457a]- This Id : {iddd} , Already Used Today Return Tomorow !"
                        	Besto_PackEt = Besto_Msg(self.EncryptedPlayerid,msg) 
                        	self.sock1200.send(bytes.fromhex(Besto_PackEt))
                        else:
                        	h1 = Get_Name(iddd)
                        	msg = f"[b][i][c][7cfc00] - Done Send Likes !\n - Name : {h1}\n - Likes Befor : [b][c]{Like1}\n - Likes After : {Like2}\n - Likes Given : [b][c]100\n - By : C4 Team "
                        	Besto_PackEt = Besto_Msg(self.EncryptedPlayerid,msg) 
                        	self.sock1200.send(bytes.fromhex(Besto_PackEt))      
                        	
                    if '1200' in data.hex()[0:4] and b'/spam' in data and self.Beston == True:           
                        i = re.split('/spam', str(data))[1]
                        #print(i)                        
                        if '***' in i:
                        	i = i.replace('***', '106')            	
                        iddd = str(i).split('(\\x')[0]
                        api = requests.get(f'https://mahmoud-aheqh0b3csgagdf4.centralus-01.azurewebsites.net/request?api_key=تيم_C4&id={iddd}&type=friend-spam')
                        nx = api.text
                        if 'Successfully processed friend-spam.':
                        	h1 = Get_Name(iddd)
                        	msg = f"[b][i][c][00ffff] - Done Send Requests!\n - Id : {iddd}\n - Name : {h1}\n - Spam Given : [b][c]200\n - By : C4 Team "
                        	Besto_PackEt = Besto_Msg(self.EncryptedPlayerid,msg) 
                        	self.sock1200.send(bytes.fromhex(Besto_PackEt))                              	
                        else:
                        	msg = "[b][i][c][ff0000] - Error For Send Spam Requests !"
                        	Besto_PackEt = Besto_Msg(self.EncryptedPlayerid,msg) 
                        	self.sock1200.send(bytes.fromhex(Besto_PackEt))                              	
                    ...
                    if '1200' in data.hex()[0:4] and b'/C4i' in data and 700 > len(data.hex()) and self.Beston == True:
                    		pack = ' - In Sqiud'
                    		go = dataC.hex()
                    		api = requests.get(f'https://rizaky-api-mh.vercel.app/api?id={pack}&lop={go}&code=RIZAKYI')
                    		if api.status_code == 200:
                    			packt = api.text
                    		else:''
                    		msg = packt
